...
def main():
    ...
    #creamos registro de entradas y fichero
    fEntradas = open("transacciones.txt", "a+")
    #transaccion de datos
    transaccion = "{},{},{},{}\n".format(numEntradas.get("Bebé"),\
                                       numEntradas.get("Niñ@"),\
                                       numEntradas.get("Adult@"),\
                                       numEntradas.get("Jubilad@"))
    fEntradas.write(transaccion)#guardo modificación del fichero
    fEntradas.close#cierro el fichero
